...
var Info = SiteInfo{
	...
	AuthorName:  "云沐",
	AuthorDesc:  "高级Java工程师（全栈能力）· 深度交付 · 效能革新",
...
var Skills = []Skill{
	...
	},
	{
		...
		Desc:  "Spring Cloud Alibaba、Nacos 服务发现、RabbitMQ / Kafka 消息队列、Dubbo/gRPC RPC、XXL-JOB 分布式调度",
	...
	{
		Title: "数据库 / 缓存",
		Desc:  "MySQL 关系数据库、Redis 缓存、NoSQL（InfluxDB / ClickHouse / Elasticsearch）",
...
var CaseStudies = []CaseStudy{
	{
		...
		Tag:            "性能调优",
		TagColor:       "emerald",
		Summary:        "基于 Java 11/17 + Spring Boot + InfluxDB 的高频数据采集系统，集成 Dubbo/gRPC 服务通信，Flink 消费 Kafka 做实时流处理。针对单月 2076 万条数据的查询瓶颈，通过 JVM 调优 + 分批并行查询，将响应时间从 60 秒骤降至 200 毫秒。",
		Challenge:      "电力物联网平台需支撑 1 万+ 测点的高频数据采集（日采集 96 点），单月数据量达 2076 万条。历史数据查询接口响应时间长达 60 秒，严重影响业务报表生成和实时监控体验。同时需支持 Flink 实时计算与缓存架构的协同。",
		Approach:       "基于 Java 11/17 + Spring Boot 技术栈，集成 Dubbo/gRPC 实现服务间通信，运用 NIO 提升 I/O 吞吐。Flink 消费 Kafka 数据流做实时聚合，结果写入 InfluxDB。采用 Stopwatch 工具定位查询瓶颈，设计分批并行查询 + 线程池隔离 + 本地缓存优化，配合 Caffeine+Redis+DB 三级缓存架构应对实时交易决策需求。",
		Result:         "响应时间从 60 秒降至 200 毫秒，性能提升 300 倍。三级缓存架构实现毫秒级接口响应，缓存命中率 95%+，数据库压力降低 80%。系统可稳定支撑每日近亿级数据点的高并发读写。",
		TechStack:      []string{"Java 11/17", "Spring Boot", "Dubbo/gRPC", "Flink", "Kafka", "InfluxDB", "Caffeine", "Redis", "NIO"},
		Highlight:      "60s → 200ms",
	...
	},
	{
		...
		Tag:            "独立产品",
		TagColor:       "purple",
		Summary:     "从 0 到 1 独立完成产品设计、UI 实现、后端开发到部署上线全流程。前端小程序原生框架，后端 Go (Gin) + SQLite 构建 RESTful API，AI 辅助编程提升开发效率。",
		Challenge:      "需要独立交付一款完整的微信小程序，涵盖用户体系、账单记录、分类统计、数据可视化等核心功能，且要求代码结构清晰、可维护性强，具备持续迭代能力。",
		Approach:       "使用小程序原生框架，后端基于 Go (Gin) 构建 RESTful API，SQLite 嵌入式数据库实现数据持久化。系统性运用 Cursor IDE 与 OpenCode 平台辅助开发，采用组件化开发模式，确保代码复用率和可维护性。",
		Result:         "产品成功上线并稳定运行，核心功能完整可用，代码质量达到生产标准，持续迭代中。",
		TechStack:      []string{"小程序原生", "Go (Gin)", "SQLite", "Cursor", "OpenCode"},
		Highlight:      "0→1 独立交付",
